...
49
-}
squareSum :: Int -> Int -> Int  -- when to use unbounded Integer?
...
sumLast2 :: Int -> Int
sumLast2 n =
    let absN = abs n in sumLast2' absN
    where
        sumLast2' absN = last1 absN + last1 (div absN 10)   -- absN does not shadow
            where
                last1 absA = mod absA 10    -- absN here would shadow, but only from immediate outer
